...
export const BidTable = ({ bids }: { bids: { price: number; quantity: number }[] }) => {
    let currentTotal = 0; 
    const relevantBids = bids.slice(0, 15);
    const bidsWithTotal: [number, number, number][] = relevantBids.map(({ price, quantity }) => [price, quantity, currentTotal += Number(quantity)]);
    const maxTotal = relevantBids.reduce((acc, bid) => acc + Number(bid.quantity), 0);

    return <div>
        {bidsWithTotal?.map(([price, quantity, total]) => <Bid maxTotal={maxTotal} total={total} key={price} price={price.toString()} quantity={quantity.toString()} />)}
    </div>
}
